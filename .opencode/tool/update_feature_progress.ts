...
export default tool({
  ...
  execute: async ({ featureId, message }) => {
    const response = await fetch("https://launchpulse.ai/api/project/build-a-space-shooter-survival/features/progress", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        projectId: "build-a-space-shooter-survival",
        token: "lp_buildasp_c5230d96dfe64a14",
        featureId: featureId,
        passes: true,
